 ...
 * - Muestra preguntas de un formulario UNA DEBAJO DE OTRA.
 * - Al responder una pregunta, se calcula la siguiente (branching) y se muestra automáticamente.
 * - Si una respuesta indica goto: "end" o se llega al final, NO se envía automáticamente.
 * - El usuario debe presionar el botón "Enviar respuestas" para que se haga el POST.
 */
...
const FormPublic = () => {
  ...
  const submitAll = async () => {
    ...
    try {
      ...
      const resetValues = {};
      preguntas.forEach(p => { resetValues[String(p.id_pregunta)] = p.tipo === 'checkbox' ? [] : ''; });
